        ...
        public string Answer2()
        {
            ...
            foreach (string line in File.ReadLines(_inputFile))
            {
                ...
                if (m.Success)
                {
                    ...
                    LinkedList<char> tempStack = new LinkedList<char>();

                    for (int i = 0; i < cratesToMove; i++)
                    {
                        char crate = crates[fromStack - 1].Last.Value;
                        crates[fromStack - 1].RemoveLast();
                        tempStack.AddFirst(crate);
                        Console.WriteLine($"Moved {crate} from {fromStack} to temp stack");
                    }
                    for (int i = 0; i < cratesToMove; i++)
                    {
                        char crate = tempStack.First.Value;
                        tempStack.RemoveFirst();
                        crates[toStack - 1].AddLast(crate);
                        Console.WriteLine($"Moved {crate} from temp stack to {toStack}");
                    }
